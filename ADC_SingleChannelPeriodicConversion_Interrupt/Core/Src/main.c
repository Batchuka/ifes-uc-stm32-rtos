...
int main(void)
{
  ...
  // inicia a utilização do ADC
  if(HAL_ADC_Start_DMA(&hadc1, &dma_buff, 1) != HAL_OK){
	  Error_Handler();
  }

  // inicia a utilização de interrupção para a conversão ADC
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  _HAL_TIM_SET_COMPARE(&htim3, TIM_CHANNEL_1, dma_buff);
	  HAL_Delay(10);
